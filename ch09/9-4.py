...
class BPNeuralNetwork:
    ...
    def back_propagate(self, case, lable, learn):
        self.predict(case)
        # 计算输出层的误差
        output_deltas = [0.0] * self.output_n
        for k in range(self.output_n):
            error = lable[k] - self.output_cells[k]
        ...
        error = 0
        for o in range(len(lable)):
            error += 0.5 * (lable[o] - self.output_cells[o]) ** 2
        return error
    ...
    def test(self):
        ...
        print(np.dot(np.dot(cases, self.input_weights), self.output_weights))
